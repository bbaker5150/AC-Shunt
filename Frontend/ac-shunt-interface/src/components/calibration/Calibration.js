...
// Sub-navigation for the calibration steps
...
function Calibration({ showNotification }) {
    ...
    const [calibrationSettings, setCalibrationSettings] = useState({ initial_warm_up_time: 0, num_samples: 8 });
    ...
    const refreshTestPointList = useCallback(async () => {
        ...
        try {
            ...
            console.error("Failed to refresh test point list:", error);
    ...
    const performAutomaticCalculations = useCallback(async (readings, results) => {
        ...
        const allAveragesPresent = [
            results.std_dc_pos_avg, results.std_dc_neg_avg, results.std_ac_open_avg,
            results.std_ac_close_avg, results.ti_dc_pos_avg, results.ti_dc_neg_avg,
            results.ti_ac_open_avg, results.ti_ac_close_avg
        ].every(val => val !== null && val !== undefined);

        if (!allAveragesPresent) {
            console.log("Skipping full calculation: Not all readings are complete.");
            try {
                if (selectedTP) {
                    const response = await axios.put(`${API_BASE_URL}/calibration_sessions/${selectedSessionId}/test_points/${selectedTP.id}/update-results/`, {
                        eta_std: eta_std_placeholder.toFixed(5),
                        eta_ti: eta_ti_placeholder.toFixed(5),
                    });
                    setFetchedResults(response.data); // Update state with partial results
                    showNotification("Readings saved. Placeholder gain factors updated.", 'info');
                }
            } catch (e) {
                showNotification("Error saving placeholder gain factors.", 'error');
            }
            return;
        }
        ...
        const delta_std_known_placeholder = 0;
        ...
        const final_ppm = delta_std_known_placeholder + term_STD - term_UUT;
        ...
        try {
            if (selectedTP) {
                ...
                setFetchedResults(response.data); // Update state with full results
    ...
    }, [selectedSessionId, selectedTP, showNotification, refreshTestPointList]);
    ...
    const handleSaveTiReadings = useCallback(async (testPoint) => {
        ...
        const { std_ac_open_readings, std_dc_pos_readings, std_dc_neg_readings, std_ac_close_readings } = testPoint.readings;
        ...
        setTiLiveReadings({
            ac_open: tiReadings.ti_ac_open_readings, dc_pos: tiReadings.ti_dc_pos_readings,
            dc_neg: tiReadings.ti_dc_neg_readings, ac_close: tiReadings.ti_ac_close_readings,
        });

        const stripMeta = ({ created_at, updated_at, test_point, id, ...rest }) => rest;
        const updatedPayload = {
            current: testPoint.current, frequency: testPoint.frequency,
            readings: { ...stripMeta(testPoint.readings), ...tiReadings, std_ac_open_readings, std_dc_pos_readings, std_dc_neg_readings, std_ac_close_readings },
            results: stripMeta(testPoint.results || {})
        };

        try {
            const response = await axios.put(`${API_BASE_URL}/calibration_sessions/${selectedSessionId}/test_points/${testPoint.id}/`, updatedPayload);
            showNotification('TI readings saved and updated for TestPoint.', 'success');
            performAutomaticCalculations(response.data.readings, response.data.results, response.data);
        } catch (error) {
            ...
            ws.current.onmessage = (event) => {
                ...
                if (data.type === 'reading_update') {
                    ...
                    axios.get(`${API_BASE_URL}/calibration_sessions/${selectedSessionId}/test_points/`).then(response => {
                        const pointToSave = response.data.test_points.find(p => p.id === selectedTP.id);
                        if (pointToSave) {
    ...
    useEffect(() => {
        if (selectedTP && selectedSessionId) {
            const fetchAndSetDataForTP = async () => {
                try {
                    ...
                    setCalibrationSettings(pointData.settings || { initial_warm_up_time: 0, num_samples: 8 });
                    setFetchedResults(pointData.results);
                    if (pointData.results) {
                        setCorrectionInputs({ eta_std: pointData.results.eta_std || '', eta_ti: pointData.results.eta_ti || '', delta_std_known: pointData.results.delta_std_known || '' });
                        setFinalPpmDifference(pointData.results.delta_uut_ppm);
                    } else {
                        setCorrectionInputs({ eta_std: '', eta_ti: '', delta_std_known: '' });
                        setFinalPpmDifference(null);
                    }
                    if (pointData.readings) {
                        setLiveReadings({ ac_open: pointData.readings.std_ac_open_readings || [], dc_pos: pointData.readings.std_dc_pos_readings || [], dc_neg: pointData.readings.std_dc_neg_readings || [], ac_close: pointData.readings.std_ac_close_readings || [], });
                        setTiLiveReadings({ ac_open: pointData.readings.ti_ac_open_readings || [], dc_pos: pointData.readings.ti_dc_pos_readings || [], dc_neg: pointData.readings.ti_dc_neg_readings || [], ac_close: pointData.readings.ti_ac_close_readings || [], });
                    } else {
                        setLiveReadings(initialLiveReadings);
                        setTiLiveReadings(initialLiveReadings);
                    }
                } catch (error) {
                    console.error("Failed to fetch data for selected test point:", error);
                    showNotification('Failed to load data for the selected test point.', 'error');
                    setLiveReadings(initialLiveReadings);
                    setTiLiveReadings(initialLiveReadings);
                }
            };
            fetchAndSetDataForTP();
        } else {
            setLiveReadings(initialLiveReadings);
            setTiLiveReadings(initialLiveReadings);
    ...
    }, [selectedTP, selectedSessionId, showNotification, setLiveReadings, setTiLiveReadings, initialLiveReadings]);
    ...
    const handleFinalCalculation = async () => {
        ...
        const deltaStdKnown = parseFloat(correctionInputs.delta_std_known);
        ...
        const final_ppm = deltaStdKnown + term_STD - term_UUT;
        const finalPpmFormatted = final_ppm.toFixed(3);
    ...
    const handleCollectReadings = (readingKey) => {
        ...
        if (ws.current?.readyState === WebSocket.OPEN) {
            ...
            ws.current.send(JSON.stringify({
                command: 'start_collection', reading_type: `std_${readingKey}`, num_samples: numSamples,
                test_point: { current: selectedTP.current, frequency: selectedTP.frequency },
            }));
        } else {
    ...
    const buildChartData = (readings) => {
        ...
        const maxLength = Math.max(0, ...Object.values(readings).map(arr => arr ? arr.length : 0));
    ...
    const showStdLiveChart = Object.values(liveReadings).some(arr => arr && arr.length > 0) || isCollecting;
    const showTiLiveChart = Object.values(tiLiveReadings).some(arr => arr && arr.length > 0);
    ...
    return (
        <>
            {!selectedSessionId ? (
                <div className="content-area form-section-warning"><p>Please select a session to run a calibration.</p></div>
            ) : (
                <>
                    {/* --- Static Configuration Summary Card --- */}
                    <div className="content-area">
                        <h2>Configuration Summary</h2>
                        <div className="calibration-summary-bar">
                            <div className="summary-item">
                                <strong>AC Shunt Range:</strong>
                                <span>{calibrationConfigurations.ac_shunt_range || 'N/A'} A</span>
                            </div>
                            <div className="summary-item">
                                <strong>Amplifier Range:</strong>
                                <span>{calibrationConfigurations.amplifier_range || 'N/A'} A</span>
                            </div>
                            <div className="summary-item">
                                <strong>Input Current:</strong>
                                <span>{tpData?.test_points?.[0]?.current ? `${tpData.test_points[0].current} A` : 'N/A'}</span>
                            </div>
                        </div>
                    </div>

                    {/* --- Main Calibration Workflow Card --- */}
                    <div className="content-area">
                        <div className="calibration-workflow-container">
                            {/* Left Sidebar: Test Point List */}
                            <div className="test-point-sidebar">
                                <h4>Test Points</h4>
                                <div className="test-point-list">
                                    {tpData?.test_points?.length > 0 ? (
                                        tpData.test_points.map((point) => {
                                            const isSelected = selectedTP && point.id === selectedTP.id;
                                            const hasAllReadings = point.readings &&
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    {hasAllReadings && <span className="status-icon">✓</span>}
                                                </button>
                                            );
                                        })
                                    ) : (
                                        <p className="no-test-points-message">No test points generated.</p>
                                    )}
                                </div>
                            </div>

                            {/* Right Content Area: Calibration Steps */}
                            <div className="test-point-content">
                                {!selectedTP ? (
                                    <div className="placeholder-content">
                                        <h3>Select a Test Point</h3>
                                        <p>Please select a test point from the list on the left to begin the calibration process.</p>
                                    </div>
                                ) : (
                                    <>
                                        ...
                                        <div className="sub-tab-content">
                                            {activeTab === 'settings' && (
                                                <form onSubmit={handleSettingsSubmit}>
                                                    ...
                                                    <button type="submit" className="button button-primary">Save and Continue</button>
                                                </form>
                                            )}
                                                ...
                                                <>
                                                    <div className="form-section">
                                                        ...
                                                        <div className="readings-grid">
                                                            {READING_TYPES.map(({ key, label }) => (
                                                                <button key={key} onClick={() => handleCollectReadings(key)} disabled={isCollecting || !selectedTP} className="button">
                                                                    {isCollecting && currentReadingKey === key ? `Collecting... (${collectionProgress.count}/${collectionProgress.total})` : `Take ${label} Readings`}
                                                                </button>
                                                            ))}
                                                        </div>
                                                        {showStdLiveChart && (
                                                            <div className="chart-container">
                                                                <div className="chart-header">
                                                                    <h5>Live Standard Readings</h5>
                                                                    <button className="button button-danger button-small" onClick={() => setLiveReadings(initialLiveReadings)}>Clear</button>
                                                                </div>
                                                    ...
                                                    <div className="form-section">
                                                        ...
                                                        {showTiLiveChart && (
                                                            <div className="chart-container">
                                                                <div className="chart-header">
                                                                    <h5>Simulated Test Instrument Readings</h5>
                                                                    <button className="button button-danger button-small" onClick={() => setTiLiveReadings(initialLiveReadings)}>Clear</button>
                                                                </div>
