...
import { FaCog, FaCalculator } from 'react-icons/fa'; // Import the gear and calculator icons
...
const calculateRangeStats = (originalChartData, typeLabel, start, end) => {
    ...
    const targetDataset = originalChartData.datasets.find(ds => ds.label === typeLabel);
    ...
    const dataSlice = targetDataset.data.slice(startIndex, endIndex).map(p => p.y);
    if (dataSlice.length < 2) return null;

    const sum = dataSlice.reduce((acc, val) => acc + val, 0);
    const mean = sum / dataSlice.length;

    const sumSqDiff = dataSlice.reduce((acc, val) => acc + Math.pow(val - mean, 2), 0);
...
function CalibrationChart({ title, chartData, theme, chartType, onHover, syncedHoverIndex, comparisonData }) {
    ...
    const [analysisOptions, setAnalysisOptions] = useState({ type: '', start: 1, end: 1 });
    const [analysisResult, setAnalysisResult] = useState(null);

    // Effect to set default analysis options when data loads
    useEffect(() => {
        const primaryDataset = chartData?.datasets?.find(ds => ds.data && ds.data.length > 0);
        if (primaryDataset) {
            setAnalysisOptions({
    ...
    const { processedChartData, processedComparisonData } = useMemo(() => {
        const processDatasets = (dataToProcess) => {
            ...
            return dataToProcess.datasets.map(ds => {
                if (yAxisUnit === 'ppm') {
                    ...
                    const localMean = ds.data.reduce((acc, curr) => acc + curr.y, 0) / ds.data.length;
                    return {
                        ...ds,
                        data: ds.data.map(point => ({
                            ...point,
                            y: localMean === 0 ? 0 : ((point.y - localMean) / Math.abs(localMean)) * 1e6
                        }))
                    };
                }
                return ds;
        ...
        if (!chartData || !chartData.datasets || chartData.datasets.every(ds => ds.data.length === 0)) {
            return { processedChartData: { datasets: [] }, processedComparisonData: [] };
        }
        
        const finalChartDatasets = processDatasets(chartData);
        const finalComparisonDatasets = processDatasets({ datasets: comparisonData });
    ...
    const handleCalculateRange = () => {
        ...
        const result = calculateRangeStats(chartData, type, parseInt(start, 10), parseInt(end, 10));
    ...
    const options = {
            ...
            tooltip: {
                callbacks: {
                    ...
                    footer: (tooltipItems) => {
                        ...
                        const formatValue = (val) => yAxisUnit === 'ppm' ? val.toFixed(finalPpmDecimals) : val.toPrecision(finalVoltPrecision);

                        const mainDataset = processedChartData.datasets[datasetIndex];
                        if (mainDataset) {
                            const mainValue = mainDataset.data[dataIndex]?.y;
                            if (mainValue !== undefined) {
                                const chartTitle = title.includes('Standard') ? 'Standard Instrument' : 'Test Instrument';
                                footerLines.push(`${chartTitle}: ${formatValue(mainValue)} ${unitLabel}`);
                            }
                        }

                        if (processedComparisonData && processedComparisonData.length > 0) {
                            const mainLabel = mainDataset?.label;
                            const comparisonDataset = processedComparisonData.find(d => d.label === mainLabel);
                            ...
                            if (comparisonValue !== undefined) {
                                ...
                                footerLines.push(`${comparisonChartTitle}: ${formatValue(comparisonValue)} ${unitLabel}`);
                            }
                        }
                                                
                        const timestamp = chartData.datasets[datasetIndex]?.data[dataIndex]?.t;
                        if (timestamp) {
        ...
        scales: {
            y: {
                beginAtZero: false,
                title: { display: true, text: yAxisUnit === 'voltage' ? 'Voltage (V)' : 'Difference (PPM)', color: textColor },
                ticks: {
                    color: textColor,
                    callback: (value) => {
                        if (yAxisUnit === 'ppm') {
    ...
    const availableMeasurementTypes = chartData.datasets.filter(ds => ds.data && ds.data.length > 0).map(ds => ds.label);
        ...
        <div>
            <div className="summary-table-header" style={{ paddingBottom: '15px' }}>
                ...
                <div className="chart-options-container" ref={optionsMenuRef}>
                    <button title="Chart Options" className="chart-options-button" onClick={() => setIsOptionsOpen(prev => !prev)}>
                        <FaCog />
                    </button>
                    {isOptionsOpen && (
                            ...
                            <div className="chart-options-section">
                                ...
                                <div className="chart-options-form-group">
                                    ...
                                    <div className="unit-toggle">
                                        <button className={yAxisUnit === 'voltage' ? 'active' : ''} onClick={() => setYAxisUnit('voltage')}>Volts</button>
                                        <button className={yAxisUnit === 'ppm' ? 'active' : ''} onClick={() => setYAxisUnit('ppm')}>PPM</button>
                                    </div>
                                </div>
                                <div className="chart-options-form-group">
                                    {yAxisUnit === 'voltage' ? (
                                        <>
                                            ...
                                            <input
                                                ...
                                                placeholder="e.g., 4"
                                            />
                                            {voltSigFigsError && <small className="error-text" style={{ color: 'var(--danger-color)', display: 'block', marginTop: '4px' }}>{voltSigFigsError}</small>}
                                        </>
                                    ) : (
                                        <>
                                            ...
                                            <input
                                                ...
                                                placeholder="e.g., 2"
                                            />
                                            {ppmDecimalPlacesError && <small className="error-text" style={{ color: 'var(--danger-color)', display: 'block', marginTop: '4px' }}>{ppmDecimalPlacesError}</small>}
                                        </>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="chart-options-section">
                                ...
                                <div className="chart-options-form-group">
                                    ...
                                    <select name="type" value={analysisOptions.type} onChange={handleAnalysisInputChange}>
                                        {availableMeasurementTypes.map(label => <option key={label} value={label}>{label}</option>)}
                                    </select>
                                </div>
                                ...
                                <button className="button button-small" onClick={handleCalculateRange} style={{ width: '100%', margin: '5px 0 0 0' }}>
                                    <FaCalculator style={{ marginRight: '8px' }}/>Calculate PPM/Mean
                                </button>
                                {analysisResult && (
            ...
            <div style={{ height: '350px' }}>
                <ChartComponent ref={chartRef} options={options} data={processedChartData} />
            </div>
