...
function getKValueFromTDistribution(dof) {
    ...
    const kValue = kLower + (roundedDof - lowerBound) * (kUpper - kLower) / (upperBound - lowerBound);
    return kValue;
}
...
const UncertaintyBudgetTable = ({ components, onRemove, calcResults }) => {
    useEffect(() => {
        if (calcResults && window.MathJax && typeof window.MathJax.typesetPromise === 'function') {
            setTimeout(() => {
                window.MathJax.typesetPromise().catch((err) => console.error('MathJax typeset failed:', err));
            }, 0);
        }
            ...
            <React.Fragment key={title}>
                ...
                {filteredComponents.map(c => {
                    const stddevInPpm = c.calculationDetails
                        ? (c.calculationDetails.stddev / Math.abs(c.calculationDetails.avg)) * 1_000_000
                        : null;
                    ...
                    return (
                        <tr key={c.id}>
                                    ...
                                    <div className="tooltip-container">
                                        ...
                                        <span className="tooltip-text" style={{ textAlign: 'left' }}>
                                            ...
                                            <p className="tooltip-note">
                                                The <strong>absolute σ</strong> is used in the formula to calculate the final uncertainty.
                                            </p>
                                            <b>Std. Uncertainty (uᵢ): {c.value != null ? c.value.toFixed(4) : 'N/A'} ppm</b>
                                        </span>
                                    </div>
                                ) : (
                                    c.name
                            ...
                            <td className="action-cell">
                                {!c.isAuto && (
                                    <span onClick={() => onRemove(c.id)} className="delete-action" title="Remove Component">×</span>
                                )}
                            </td>
                        </tr>
                    );
    ...
    const stdComponents = components.filter(c => c.category === 'Standard Instrument');
    const uutComponents = components.filter(c => c.category === 'Unit Under Test (UUT)');
    const sysComponents = components.filter(c => c.category === 'System & Environmental');
                ...
                {calcResults && (
                    <>
                        <tr>
                            ...
                            <td>{(calcResults.effectiveDof === Infinity || calcResults.effectiveDof === null) ? '∞' : calcResults.effectiveDof.toFixed(2)}</td>
                            <td colSpan="2"></td>
                        </tr>
                        <tr>
            ...
            {!calcResults ? (
                ...
                <div style={{textAlign: 'center'}}>
                    ...
                    <ul className="result-breakdown">
                        <li><span className="label">Combined Uncertainty (uₑ)</span><span className="value">{calcResults.combinedUncertainty.toFixed(4)} ppm</span></li>
                        <li><span className="label">Effective DoF (vₑₒₒ)</span><span className="value">{(calcResults.effectiveDof === Infinity || calcResults.effectiveDof === null) ? '∞' : calcResults.effectiveDof.toFixed(2)}</span></li>
                        <li><span className="label">Coverage Factor (k)</span><span className="value">{calcResults.kValue.toFixed(3)}</span></li>
                    </ul>
                    <p className="result-confidence-note">
                        The reported expanded uncertainty of measurement is stated as the standard uncertainty of measurement multiplied by the coverage factor k={calcResults.kValue.toFixed(3)}, which for a t-distribution with vₑₒₒ = {(calcResults.effectiveDof === Infinity || calcResults.effectiveDof === null) ? '∞' : calcResults.effectiveDof.toFixed(2)} corresponds to a coverage probability of approximately 95%.
                    </p>
                </div>
            )}
        </>
        ...
        <div className="modal-overlay">
            <div className="modal-content breakdown-modal-content">
                ...
                <div className="breakdown-step">
                    ...
                    1. Deviation Uncertainty (uₔₑᵥ): {`$$ u_{dev} = \\frac{L_{Upper}}{\\Phi^{-1}((1+R)/2)} = \\frac{${LUp_symmetric.toFixed(2)}}{\\Phi^{-1}((1+${inputs.reliability})/2)} = ${results.uDev.toFixed(4)} \\text{ ppm} $$`}
                    2. UUT Uncertainty: {`$$ u_{UUT} = \\sqrt{u_{dev}^2 - u_{cal}^2} = \\sqrt{${results.uDev.toFixed(4)}^2 - ${results.uCal.toFixed(4)}^2} = \\mathbf{${results.uUUT.toFixed(4)}} \\text{ ppm} $$`}
                </div>
                <div className="breakdown-step">
                    <h5>Acceptance Limits (A)</h5>
                    <p>Calculated by applying the **Guard Band Multiplier** to the tolerance limits.</p>
                    {`$$ A_{Low} = L_{Low} \\times G = ${inputs.LLow.toFixed(2)} \\times ${inputs.guardBandMultiplier} = \\mathbf{${results.ALow.toFixed(4)}} \\text{ ppm} $$`}
                    {`$$ A_{Up} = L_{Up} \\times G = ${inputs.LUp.toFixed(2)} \\times ${inputs.guardBandMultiplier} = \\mathbf{${results.AUp.toFixed(4)}} \\text{ ppm} $$`}
                </div>
                 <div className="breakdown-step">
                    <h5>Correlation (ρ)</h5>
                    <p>The statistical correlation between the UUT's true value and the measured value.</p>
                    {`$$ \\rho = \\frac{u_{UUT}}{u_{dev}} = \\frac{${results.uUUT.toFixed(4)}}{${results.uDev.toFixed(4)}} = \\mathbf{${results.correlation.toFixed(4)}} $$`}
                </div>
            </div>
        </div>
    );
        ...
        <div className="modal-overlay">
            <div className="modal-content breakdown-modal-content">
                ...
                <div className="breakdown-step">
                    <h5>Step 2: Inputs</h5>
                    <ul>
                        <li>Tolerance Span: {`$$ L_{Upper} - L_{Lower} = ${inputs.LUp.toFixed(2)} - (${inputs.LLow.toFixed(2)}) = ${(inputs.LUp - inputs.LLow).toFixed(2)} \\text{ ppm} $$`}</li>
                        <li>Expanded Uncertainty: {`$$ U_{95} = ${results.expandedUncertainty.toFixed(4)} \\text{ ppm} $$`}</li>
                    </ul>
                </div>
                <div className="breakdown-step">
                    <h5>Step 3: Final Calculation</h5>
                    {`$$ TUR = \\frac{${(inputs.LUp - inputs.LLow).toFixed(2)}}{${results.expandedUncertainty.toFixed(4)}} = \\mathbf{${results.tur.toFixed(4)}:1} $$`}
                </div>
            </div>
        </div>
    );
        ...
        <div className="modal-overlay">
            <div className="modal-content breakdown-modal-content">
                ...
                <div className="breakdown-step">
                    <h5>Step 2: Inputs</h5>
                    <ul>
                        <li>UUT Tolerance Span: {`$$ ${inputs.LUp.toFixed(2)} - (${inputs.LLow.toFixed(2)}) = \\mathbf{${uutToleranceSpan.toFixed(2)}} \\text{ ppm} $$`}</li>
                        <li>TMDE Tolerance Span: {`$$ \\mathbf{${tmdeToleranceSpan.toFixed(2)}} \\text{ ppm} $$`} <em>(Derived from the 'Standard Instrument' component in the budget)</em></li>
                    </ul>
                </div>
                <div className="breakdown-step">
                    <h5>Step 3: Final Calculation</h5>
                    {`$$ TAR = \\frac{${uutToleranceSpan.toFixed(2)}}{${tmdeToleranceSpan.toFixed(2)}} = \\mathbf{${results.tar.toFixed(4)}:1} $$`}
                </div>
            </div>
        </div>
    );
        ...
        <div className="modal-overlay">
            <div className="modal-content breakdown-modal-content">
                ...
                <div className="breakdown-step">
                    ...
                    {`$$ z_{L_{Low}} = ${z1.toFixed(4)}, z_{L_{Up}} = ${z4.toFixed(4)}, z_{A_{Low}} = ${z3.toFixed(4)}, z_{A_{Up}} = ${z2.toFixed(4)} $$`}
                </div>
                <div className="breakdown-step">
                    ...
                    Term A: {`$$ \\Phi_2(${z1.toFixed(2)}, ${z2.toFixed(2)}, \\rho) = ${(bivariateNormalCDF(z1, z2, results.correlation)).toFixed(6)} $$`}
                    Term B: {`$$ \\Phi_2(${z1.toFixed(2)}, ${z3.toFixed(2)}, \\rho) = ${(bivariateNormalCDF(z1, z3, results.correlation)).toFixed(6)} $$`}
                    Term C: {`$$ \\Phi_2(${z4.toFixed(2)}, ${z5.toFixed(2)}, \\rho) = ${(bivariateNormalCDF(z4, z5, results.correlation)).toFixed(6)} $$`}
                    Term D: {`$$ \\Phi_2(${z4.toFixed(2)}, ${z6.toFixed(2)}, \\rho) = ${(bivariateNormalCDF(z4, z6, results.correlation)).toFixed(6)} $$`}
                </div>
                <div className="breakdown-step">
                    <h5>Step 4: Final PFA Calculation</h5>
                    Lower Tail Risk (A-B): {`$$ ${(results.pfa_term1 / 100).toFixed(6)} $$`}
                    Upper Tail Risk (C-D): {`$$ ${(results.pfa_term2 / 100).toFixed(6)} $$`}
        ...
        <div className="modal-overlay">
            <div className="modal-content breakdown-modal-content">
                ...
                <div className="breakdown-step">
                    ...
                    <p>Using Z-scores and correlation (ρ = {results.correlation.toFixed(4)}), we solve the Bivariate Normal CDF (Φ₂).</p>
                    Term A: {`$$ \\Phi_2(${z1.toFixed(2)}, ${z2.toFixed(2)}, \\rho) = ${(bivariateNormalCDF(z1, z2, results.correlation)).toFixed(6)} $$`}
                    Term B: {`$$ \\Phi_2(${z3.toFixed(2)}, ${z2.toFixed(2)}, \\rho) = ${(bivariateNormalCDF(z3, z2, results.correlation)).toFixed(6)} $$`}
                    Term C: {`$$ \\Phi_2(${z4.toFixed(2)}, ${z5.toFixed(2)}, \\rho) = ${(bivariateNormalCDF(z4, z5, results.correlation)).toFixed(6)} $$`}
                    Term D: {`$$ \\Phi_2(${z6.toFixed(2)}, ${z5.toFixed(2)}, \\rho) = ${(bivariateNormalCDF(z6, z5, results.correlation)).toFixed(6)} $$`}
                </div>
                <div className="breakdown-step">
                    <h5>Step 3: Final PFR Calculation</h5>
                    Lower Side Risk (A-B): {`$$ ${(results.pfr_term1 / 100).toFixed(6)} $$`}
                    Upper Side Risk (C-D): {`$$ ${(results.pfr_term2 / 100).toFixed(6)} $$`}
...
const RiskAnalysisDashboard = ({ results, onShowBreakdown }) => {
    useEffect(() => {
        if (results && window.MathJax) { window.MathJax.typesetPromise(); }
    }, [results]);
...
function Analysis({ testPointData, showNotification, selectedSessionId, onDataSave }) {
    ...
    // ===================================================================
    // =========== NEW, REFACTORED STATE FOR TYPE B COMPONENT ============
    // ===================================================================
    const [newComponent, setNewComponent] = useState({
        name: '',
        type: 'B',
        distribution: 'uniform',
        toleranceLimit: '', // For uniform/triangular
        expandedUncertainty: '', // For normal
        coverageFactor: 2, // For normal
        dof: Infinity,
    ...
    // ===================================================================
    // =========== REMOVED stdLLow/stdLUp FROM RISK INPUTS STATE =========
    // ===================================================================
    ...
    useEffect(() => {
        if (results && results.is_detailed_uncertainty_calculated) {
            setCalcResults({
                ...
                effectiveDof: results.effective_dof === null ? Infinity : results.effective_dof,
    ...
    const basicSpecificationUncertainty = useMemo(() => {
        ...
        const freqPoints = Object.keys(specRow).filter(k => k !== 'DC').map(Number).sort((a, b) => a - b);
        if (freq >= freqPoints[freqPoints.length - 1]) return specRow[freqPoints[freqPoints.length - 1].toString()];
        ...
        return s_lower + (freq - f_lower) * (s_upper - s_lower) / (f_upper - f_lower);
    }, [shuntKey, testPointInfo]);

    const saveManualComponents = useCallback(async (updatedComponents) => {
        const payload = { manual_uncertainty_components: updatedComponents };
        const testPointId = testPointInfo.testPointId;
        try {
            await axios.put(`${API_BASE_URL}/calibration_sessions/${selectedSessionId}/test_points/${testPointId}/update-results/`, payload);
        } catch (error) {
    ...
    // ===================================================================
    // =========== NEW, REFACTORED handleAddComponent FUNCTION ===========
    // ===================================================================
    const handleAddComponent = () => {
        ...
        switch (newComponent.distribution) {
            ...
            case 'triangular':
                ...
                if (isNaN(tol) || tol <= 0) {
                    showNotification('Please provide a valid, positive tolerance limit.', 'warning');
                    return;
                }
                value = newComponent.distribution === 'uniform' ? tol / Math.sqrt(3) : tol / Math.sqrt(6);
                toleranceLimit = tol;
                break;
            case 'normal':
                ...
                if (isNaN(expUnc) || isNaN(k) || expUnc <= 0 || k <= 0) {
                    showNotification('Please provide valid expanded uncertainty and coverage factor values.', 'warning');
                    return;
                }
                value = expUnc / k;
                toleranceLimit = expUnc; // Use expanded uncertainty as the effective tolerance for TAR
                break;
            default:
        ...
        if (!newComponent.name || value === null) {
            showNotification('Component name and a valid uncertainty value are required.', 'warning');
            return;
        }
        ...
        const componentToAdd = {
            ...
            type: 'B',
            value: value, // This is the standard uncertainty in ppm
            dof: isNaN(dof) || dof <= 0 ? Infinity : dof,
            isAuto: false,
            // Store original inputs for clarity and future use (e.g., TAR)
            distribution: newComponent.distribution,
            toleranceLimit: toleranceLimit,
            coverageFactor: newComponent.distribution === 'normal' ? newComponent.coverageFactor : null,
        ...
        // Reset form
        setNewComponent({ name: '', type: 'B', distribution: 'uniform', toleranceLimit: '', expandedUncertainty: '', coverageFactor: 2, dof: Infinity, category: 'System & Environmental' });
    };
    ...
    const handleInputChange = (e) => setNewComponent(prev => ({ ...prev, [e.target.name]: e.target.value }));
    ...
    const handleCalculateUncertainty = async () => {
        ...
        const payload = {
            ...
            effective_dof: newResults.effectiveDof === Infinity ? null : newResults.effectiveDof,
    ...
    // ===================================================================
    // =========== NEW, REFACTORED calculateRiskMetrics FUNCTION =========
    // ===================================================================
    const calculateRiskMetrics = () => {
        ...
        if (isNaN(reliability) || reliability <= 0 || reliability >= 1) {
            showNotification('Please enter a valid reliability (e.g., 0.95).', 'warning');
            return;
        }
        if (isNaN(guardBandMultiplier) || guardBandMultiplier <= 0 || guardBandMultiplier > 1) {
            showNotification('Guard Band Multiplier must be between 0 and 1.', 'warning');
            return;
        }
        if (!calcResults) {
        ...
        let uUUT = 0;
        if (uUUT2 <= 0) {
            showNotification('Warning: UUT uncertainty is non-positive. Test uncertainty may be too large for the specified reliability and tolerance.', 'warning');
            uUUT = 0;
        } else {
    ...
    if (!results || results.delta_uut_ppm === null) {
        return <div className="content-area"><p>The final AC-DC Difference has not been calculated. Please complete the calculation on the "Run Calibration" tab.</p></div>;
    }
    ...
    const renderSpecComparison = () => {
        ...
        const GaugeBackground = () => (
            <svg className="gauge-svg-background" viewBox="-5 -5 210 115" preserveAspectRatio="none">
                <path d="M 10 100 A 90 90 0 0 1 163.64 36.36" className="gauge-arc-good" />
                <path d="M 163.64 36.36 A 90 90 0 0 1 177.94 55" className="gauge-arc-warning" />
            ...
            <div className={`spec-dashboard ${statusClass}`}>
                <div className="spec-gauge-container">
                    <div className="spec-gauge">
                        <GaugeBackground />
                        <div className="gauge-needle" style={{ transform: `rotate(${needleAngle}deg)` }}></div>
                        <div className="gauge-center-pivot"></div>
                    </div>
                    <div className="gauge-value">{percentageOfSpec.toFixed(1)}<span>%</span></div>
                ...
                <div className="spec-details-container">
                    <div className="spec-detail-card user-spec">
                        ...
                        <span className="detail-sub-value">k ≈ {calcResults.kValue.toFixed(2)}</span>
                    </div>
    ...
    return (
        <div>
            ...
            <div className="view-toggle" style={{ justifyContent: 'center', marginBottom: '30px' }}>
                ...
                <button className={analysisMode === 'basic' ? 'active' : ''} onClick={() => setAnalysisMode('basic')}>Specification Comparison</button>
            </div>
            ...
            {analysisMode === 'detailed' && (
              <div className="analysis-dashboard">
                        ...
                        <Accordion title="Add Other Type B Component">
                            <div className="config-grid" style={{ paddingTop: '15px' }}>
                                ...
                                {newComponent.distribution === 'normal' && (
                                    <>
                                        <div className="config-column"><label>Expanded Uncertainty (± ppm)</label><input type="number" step="any" name="expandedUncertainty" value={newComponent.expandedUncertainty} onChange={handleInputChange} placeholder="e.g., 50" /></div>
                                        <div className="config-column"><label>Coverage Factor (k)</label><input type="number" step="any" name="coverageFactor" value={newComponent.coverageFactor} onChange={handleInputChange} /></div>
                                    </>
                                )}
                                <div className="config-column"><label>Degrees of Freedom</label><input type="number" step="1" min="1" name="dof" value={newComponent.dof} onChange={handleInputChange} placeholder="Infinity" /></div>
                            </div>
                    ...
                    <div className="configuration-panel">
                        <Accordion title="Uncertainty Budget (AC-DC Difference, ppm)" startOpen={true}>
                            <div className="uncertainty-legend">
                                ...
                                <p>For Type A components, this is calculated from the standard deviation of the mean of the measurement data.</p>
                                <p style={{ textAlign: 'center', fontFamily: 'serif', fontSize: '1.2rem', margin: '10px 0' }}>{`$$ u_i = \\frac{\\sigma / \\sqrt{n}}{|Avg|} \\times 10^6 $$`}</p>
                                <hr />
                                <h5>Degrees of Freedom (vᵢ)</h5>
                                <p>Represents the statistical reliability of an uncertainty component. For Type A, it's the number of samples minus one (n-1). For most Type B estimates, it is considered infinite.</p>
                                <hr />
                                <h5>Effective Degrees of Freedom (v_eff)</h5>
                                <p>The effective degrees of freedom for the combined uncertainty is calculated using the Welch-Satterthwaite equation:</p>
                                <p style={{ textAlign: 'center', fontFamily: 'serif', fontSize: '1.2rem', margin: '10px 0' }}>{`$$v_{eff} = \\frac{u_c^4}{\\sum_{i=1}^{N} \\frac{u_i^4}{v_i}}$$`}</p>
                            </div>
                    ...
                    <div className="calculation-options" style={{gridColumn: '1 / -1'}}>
                        ...
                        <button onClick={handleCalculateUncertainty} className="button" style={{ fontSize: '1.1rem', padding: '12px 30px', margin: '0 0 0 20px' }}>Calculate & Save Uncertainty</button>
                    </div>
                </div>
            )}
                        ...
                        <>
                            ...
                            <div className="risk-inputs-container">
                                <div className="config-column">
                                    ...
                                    <input type="number" name="LLow" value={riskInputs.LLow} onChange={handleRiskInputChange} placeholder="e.g., -100" />
                                </div>
                                <div className="config-column">
                                    <label>UUT Upper Tolerance Limit (LUp)</label>
                                    <input type="number" name="LUp" value={riskInputs.LUp} onChange={handleRiskInputChange} placeholder="e.g., 100" />
                                </div>
                                <div className="config-column">
...
function UncertaintyAnalysis({ showNotification }) {
    const { selectedSessionId } = useInstruments();
    const [testPoints, setTestPoints] = useState([]);
    const [selectedTestPointId, setSelectedTestPointId] = useState('');
    ...
    const fetchTestPoints = useCallback(async () => {
        ...
        setIsLoading(true);
        try {
            const response = await axios.get(`${API_BASE_URL}/calibration_sessions/${selectedSessionId}/test_points/`);
            const loadedPoints = response.data?.test_points || [];

            const pointGroups = new Map();
            loadedPoints.forEach(p => {
                const key = `${p.current}-${p.frequency}`;
                if (!pointGroups.has(key)) {
                    pointGroups.set(key, { current: p.current, frequency: p.frequency });
                }
            ...
            showNotification('Could not load test points for analysis.', 'error');
        } finally {
            setIsLoading(false);
        }
    ...
    useEffect(() => {
        ...
        if (pointGroup) {
            ...
            setTestPointData({
                ...
                testPointInfo: {
                    current: pointGroup.current,
                    frequency: formatFrequency(pointGroup.frequency),
                    frequencyValue: pointGroup.frequency,
                    direction: 'Averaged',
    ...
    return (
        <div className="content-area uncertainty-analysis-page">
            ...
            {!selectedSessionId ? (
                <div className="form-section-warning"><p>Please select a session from the "Session Setup" tab to perform analysis.</p></div>
            ) : isLoading ? (
                <div className="content-area"><p>Loading test point data...</p></div>
            ) : testPoints.length === 0 ? (
                <div className="form-section-warning"><p>No test points with a final averaged result are available in this session.</p></div>
            ) : (
                <div className="results-workflow-container">
                    <aside className="results-sidebar">
                        <h4>Completed Test Points</h4>
                        <div className="test-point-list">
                            {testPoints.map(tp => {
                                const key = `${tp.current}-${tp.frequency}`;
                                const isSelected = selectedTestPointId === key;
                                const isComplete = !!(tp.forward?.results?.delta_uut_ppm_avg);

                                return (
                                    <button
                                        key={key}
                                        onClick={() => setSelectedTestPointId(key)}
                                        className={`test-point-item ${isSelected ? 'active' : ''} ${isComplete ? 'completed' : ''}`}
                                    >
                                        <span>{tp.current}A @ {formatFrequency(tp.frequency)}</span>
                    ...
                    <main className="results-content">
                        {testPointData ? (
                            ...
                            <div className="placeholder-content">
                                ...
                                <p>Please select a completed test point from the list to begin the uncertainty analysis.</p>
                            </div>
                        )}
                    </main>
                </div>
