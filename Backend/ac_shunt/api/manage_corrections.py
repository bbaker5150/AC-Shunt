...
def process_shunt_data(xls):
    ...
    This function extracts data from pairs of Corrections/Uncertainties sheets,
    merges them, and cleans the data by unpivoting frequency columns. It then 
    extracts model name and serial number from a 'Remarks' column and formats 
    ...
    Returns:
        list[dict] or None: A list of dictionaries representing all shunt 
                            correction records. Returns None on critical error.
    """
    all_records = []
    
    # Define the sheet pairs to process
    sheet_pairs = [
        ('AC Shunt Corrections', 'AC Shunt Uncertainties'),
        ('Y5020 Corrections', 'Y5020 Uncertainties')
    ]

    for corr_sheet, unc_sheet in sheet_pairs:
        # Gracefully skip if an older workbook is uploaded without the new sheets
        if corr_sheet not in xls.sheet_names or unc_sheet not in xls.sheet_names:
            print(f"Notice: Sheets '{corr_sheet}' or '{unc_sheet}' not found in workbook. Skipping.")
            continue
            
        try:
            print(f"Processing sheets: {corr_sheet} & {unc_sheet}...")
            df_corr = pd.read_excel(xls, sheet_name=corr_sheet, header=1)
            df_unc = pd.read_excel(xls, sheet_name=unc_sheet, header=1)
            
            df_corr = df_corr.loc[:, ~df_corr.columns.duplicated()]
            df_unc = df_unc.loc[:, ~df_unc.columns.duplicated()]

            # --- FIX 1: Robust Column Detection ---
            # Prevents crashes if you add a frequency to only one sheet
            def get_vars(df):
                i_vars, f_vars = [], []
                for col in df.columns:
                    try:
                        ...
                        i_vars.append(col)
                return i_vars, f_vars

            corr_id_vars, corr_freq_vars = get_vars(df_corr)
            unc_id_vars, unc_freq_vars = get_vars(df_unc)
            
            melted_corr = df_corr.melt(id_vars=corr_id_vars, value_vars=corr_freq_vars, var_name='frequency', value_name='correction').dropna(subset=['correction'])
            melted_unc = df_unc.melt(id_vars=unc_id_vars, value_vars=unc_freq_vars, var_name='frequency', value_name='uncertainty').dropna(subset=['uncertainty'])
            
            # Merge safely using only the ID columns that exist in both sheets
            merge_cols = [col for col in corr_id_vars if col in unc_id_vars] + ['frequency']
            df = pd.merge(melted_corr, melted_unc, on=merge_cols, how='outer')
            
            # --- FIX 2: Sanitize Data for SQL Server ---
            df['correction'] = df['correction'].astype(object).where(pd.notnull(df['correction']), None)
            df['uncertainty'] = df['uncertainty'].astype(object).where(pd.notnull(df['uncertainty']), None)
            ...
            print(f"Extracted {len(records)} records from {corr_sheet}.")
            all_records.extend(records)
            
        except Exception as e:
            print(f"Error processing sheet pair {corr_sheet}/{unc_sheet}: {e}")
            continue

    if not all_records:
        return None
        
    print(f"Shunt data processing: Found a total of {len(all_records)} valid correction records across all sheets.")
    return all_records
